...
const SinCosSample gSinCosTbl[ 257 ] = {
...
f32 AtanFIdx_( f32 x )
{
    x *= 32.0f;
    u16 idx = F32ToU16( x );
    f32 val; // DWARF specifies this is declared before r
...
{
    f32 abs_fidx;
    for( abs_fidx = FAbs( fidx ); abs_fidx > 65536.0f; abs_fidx -= 65536.0f )
    {
    }
    ...
    val = detail::gSinCosTbl[ static_cast< u8 >( idx ) ].sin_val +
            r * detail::gSinCosTbl[ static_cast< u8 >( idx ) ].sin_delta;
    return fidx < 0.0f ? -val : val;
}

f32 CosFIdx( f32 fidx )
{
    for( fidx = FAbs( fidx ); fidx > 65536.0f; fidx -= 65536.0f )
    {
    }

    u16 idx = F32ToU16( fidx );
    f32 r = fidx - U16ToF32( idx );
    return detail::gSinCosTbl[ static_cast< u8 >( idx ) ].cos_val +
            r * detail::gSinCosTbl[ static_cast< u8 >( idx ) ].cos_delta;
}
...
void SinCosFIdx( register f32 *pSin, register f32 *pCos, register f32 fidx )
{
    ...
    register __vec2x32float__ scval, scdel, result;
    register f32 c_zero;
    register const f32 *pTbl = reinterpret_cast< const f32 * >( &detail::gSinCosTbl );

    ASM
    (
        fabs abs_fidx, fidx;
        psq_st abs_fidx, 0x0( pSin ), 1, 3;
